...
namespace MYARCH.SERVICES.Services
{
    public class BagimsizBolumlerService : IBagimsizBolumlerService
    {
        ...
        public EBagimsizBolumlerDTO BagimsizBolumBilgileriGetir(int id)
        {
            var control = (from u in _bagimsizBolumlerRepository.GetAll()
                           ...
                           select new EBagimsizBolumlerDTO
                           {
                               ...
                               katmaliki = u.katmaliki,
                               katmaliki_id = u.katmaliki_id,
                               kiracı = u.kiracı,
                               kiraci_id = u.kiraci_id,
                               daire_katsayisi = u.daire_katsayisi
        ...
        public bool BagimsizBolumGuncelle(bagimsiz_bolumler model)
        {
            ...
            model.katmaliki_id = gelenBagimsizBolum.katmaliki_id;
            model.katmaliki = gelenBagimsizBolum.katmaliki;
            model.kiraci_id = gelenBagimsizBolum.kiraci_id;
            model.kiracı = gelenBagimsizBolum.kiracı;
            ...
            using (var context = new MyArchContext())
            {
               ...
               int sonuc = context.Database.ExecuteSqlCommand("UPDATE bagimsiz_bolum_kisiler SET daire_katsayisi = '" + sonKatsayisi + "' WHERE bagimsiz_id = '" + model.id + "'");
            }
        ...
        public List<EBagimsizBolumlerDTO> BagimsizBolumler()
          {
            try
            {
                using (var context = new MyArchContext())
                {
                    ...
                    var bagimsizBolumler = context.Database.SqlQuery<EBagimsizBolumlerDTO>("select id,daire_katsayisi,kiraci_id,katmaliki_id,blok_adi,daire_numarasi,katmaliki,kiracı,toplam,su_abone_no,elektrik_abone_no,petek_boyu,kat from bagimsiz_bolumler as bb left join (select sum(borc-alacak) as toplam,bagimsiz_id from hesap_hareket group by bagimsiz_id) as brc on bb.id=brc.bagimsiz_id where aktif IS NULL ").ToList();
        ...
        public bool BagimsizBolumKaydet(bagimsiz_bolumler model)
        {
            try
            {
                _bagimsizBolumlerRepository.Insert(model);
                int kayit = _uow.SaveChanges();
                if (kayit>0)
                {
                    return true;
                }
